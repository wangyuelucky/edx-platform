...
class CrowdsourceHinterModule(CrowdsourceHinterFields, XModule):
    ...
    - Display how many votes each hint got, after the user votes.
    - Move this HTML string manipulation to a mako template.
    - Profanity and answer-giveaway filters
    '''
    ...
    def get_hint(self, get):
        ...
        hint_text = '<b>Hints:</b></br>' + best_hint + '<br />' + rand_hint_1 + '<br />' + rand_hint_2
    ...
    def get_feedback(self, get):
        ...
        for i in xrange(len(self.previous_answers)):
            ...
            if answer in self.hints:
                ...
                # Let the student create his own hint.
                out += 'Write a hint for other students who got the wrong answer of ' + answer + '.'
            out += '<textarea cols="50" class="custom-hint" style="height:200px" id="custom-hint-'+str(i)+'''">
What would you say to help someone who got this wrong answer?
(Don't give away the answer, please.)
                </textarea>'''

            out += '<br /><input class="submit-hint" data-answer="' + str(i) + '" type="button" value="Submit">'

            # Close the .previous-answer div.
            out += '</div>'

        # Add preamble.
        out2 = '''Help us improve our hinting system!
            Start by picking one of your previous incorrect answers from below: <br />
